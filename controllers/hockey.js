...
const getAllFixtures = async (req, res) => {
  ...
  if (todayFixture) {
    ...
    try {
      // get leagues first
      const leaguesResponse = await LeaguesModel.find({
        sportType: SPORTTYPE,
        season: CURRENTYEAR,
      });

      const leagues = leaguesResponse[0].data.response;
      ...
      for (let league = 0; league < batchLeagues.length; league++) {
        ...
        console.log("fetching from =>", leagueID);
  ...
  if (todayOdds) {
    ...
    try {
      ...
      for (let i = 0; i < activeGames.length; i++) {
        ...
        console.log("fetching from => ", leagueName);
        // Make request for the current page
...
const getAToZ = async (req, res) => {
  try {
    ...
    const preparedData = await extractCountry2(leaguesArray, activeFixtures);
...
const getYesterdayFixtures = async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < batchLeagues.length; i++) {
      ...
      console.log("Hockey Fetching Yesterday Fixtures => ", leagueID);
...
const getTomorrowFixtures = async (req, res) => {
  const tomorrowFixture = await FixturesModel.findOne({
    date: TOMMOROW,
    sportType: SPORTTYPE,
  });
